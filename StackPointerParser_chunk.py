...
def main():
    ...
    args_parser.add_argument('--mode', choices=['RNN', 'LSTM', 'GRU', 'FastLSTM'], help='architecture of rnn', default='FastLSTM')
    args_parser.add_argument('--num_epochs', type=int, default=10, help='Number of training epochs')  # check from paper
    args_parser.add_argument('--batch_size', type=int, default=32, help='Number of sentences in each batch')  # check from paper
    args_parser.add_argument('--decoder_input_size', type=int, default=256, help='Number of input units in decoder RNN.')  # check from paper
    args_parser.add_argument('--hidden_size', type=int, default=256, help='Number of hidden units in RNN')  # check from paper
    ...
    args_parser.add_argument('--encoder_layers', type=int, default=1, help='Number of layers of encoder RNN')  # check from paper
    args_parser.add_argument('--decoder_layers', type=int, default=1, help='Number of layers of decoder RNN')  # check from paper
    args_parser.add_argument('--char_num_filters', type=int, default=50, help='Number of filters in CNN(Character Level)')  # original one
    args_parser.add_argument('--eojul_num_filters', type=int, default=100, help='Number of filters in CNN(Eojul Level)')  # new in this version
    ...
    args_parser.add_argument('--eojul', action='store_true', help='use eojul embedding and CNN.')  # new in this version
    args_parser.add_argument('--word_dim', type=int, default=100, help='Dimension of Word embeddings')  # new in this version
    ...
    args_parser.add_argument('--learning_rate', type=float, default=0.001, help='Learning rate')  # check from paper
    ...
    args_parser.add_argument('--p_rnn', nargs=2, type=float, default=[0.33, 0.33], help='dropout rate for RNN')
    ...
    args_parser.add_argument('--schedule', type=int, default=20, help='schedule for learning rate decay')
    args_parser.add_argument('--unk_replace', type=float, default=0., help='The rate to replace a singleton word with UNK')
    args_parser.add_argument('--punctuation', nargs='+', type=str, help='List of punctuations')
    args_parser.add_argument('--beam', type=int, default=1, help='Beam size for decoding')  # study
    args_parser.add_argument('--word_embedding', choices=['random', 'word2vec', 'glove', 'senna', 'sskip', 'polyglot'], help='Embedding for words', required=True)
    args_parser.add_argument('--word_path', help='path for word embedding dict')
    args_parser.add_argument('--freeze', action='store_true', help='frozen the word embedding (disable fine-tuning).')
    args_parser.add_argument('--char_embedding', choices=['random', 'word2vec'], help='Embedding for characters', required=True)
    args_parser.add_argument('--char_path', help='path for character embedding dict')
    args_parser.add_argument('--pos_embedding', choices=['random', 'word2vec'], help='Embedding for part of speeches', required=True)  # new in this version
    args_parser.add_argument('--pos_path', help='path for part of speech embedding dict')  # new in this version
    args_parser.add_argument('--train')  # "data/train.conllu"
    args_parser.add_argument('--dev')  # "data/dev.conllu"
    args_parser.add_argument('--test')  # "data/test.conllu"
    ...
    data_dev = conllx_stacked_data.read_stacked_data_to_variable(dev_path, word_alphabet, char_alphabet, pos_alphabet, type_alphabet, use_gpu=use_gpu, prior_order=prior_order)
    ...
    def construct_pos_embedding_table():  # new in this version
        if pos_dict is None:
            return None

        scale = np.sqrt(3.0 / pos_dim)
        table = np.empty([num_pos, pos_dim], dtype=np.float32)
        table[conllx_stacked_data.UNK_ID, :] = np.random.uniform(-scale, scale, [1, pos_dim]).astype(np.float32)
        oov = 0
        for pos, index in pos_alphabet.items():
            if pos in pos_dict:
                embedding = pos_dict[pos]
            else:
                embedding = np.random.uniform(-scale, scale, [1, pos_dim]).astype(np.float32)
                oov += 1
            table[index, :] = embedding
        print('pos OOV: %d' % oov)
        return torch.from_numpy(table)
    ...
    pos_table = construct_pos_embedding_table()  # new in this version

    char_window = 3
    eojul_window = 3  # new in this version
    ...
    logger.info("Eojul CNN: filter=%d, kernel=%d" % (eojul_num_filters, eojul_window))  # new in this version
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for batch in range(1, num_batches + 1):
            input_encoder, input_decoder = conllx_stacked_data.get_batch_stacked_variable(data_train, batch_size, unk_replace=unk_replace, use_gpu=use_gpu)
            word_c, word_f, char_c, char_f, pos_c, pos_f, heads, types, masks_e, lengths_e = input_encoder
            ...
            loss_cov, num_leaf, num_non_leaf = network.loss(word_c, word_f, char_c, char_f, pos_c, pos_f, heads, stacked_heads, children, sibling, stacked_types, label_smooth,
            ...
            train_err_arc_leaf += loss_arc_leaf.item() * num_leaf  ##180809 data[0] --> item()
            train_err_arc_non_leaf += loss_arc_non_leaf.item() * num_non_leaf  ##180809 data[0] --> item()

            train_err_type_leaf += loss_type_leaf.item() * num_leaf  ##180809 data[0] --> item()
            train_err_type_non_leaf += loss_type_non_leaf.item() * num_non_leaf  ##180809 data[0] --> item()

            train_err_cov += loss_cov.item() * (num_leaf + num_non_leaf)  ##180809 data[0] --> item()
        ...
        dev_total_root = 0.0
        dev_total_inst = 0.0
        for batch in conllx_stacked_data.iterate_batch_stacked_variable(data_dev, batch_size, use_gpu=use_gpu):
            input_encoder, _, sentences = batch
            word_c, word_f, char_c, char_f, pos_c, pos_f, heads, types, masks, lengths = input_encoder
            heads_pred, types_pred, _, _ = network.decode(word_c, word_f, char_c, char_f, pos_c, pos_f, mask=masks, length=lengths, beam=beam, leading_symbolic=conllx_stacked_data.NUM_SYMBOLIC_TAGS)

            word_c = word_c.data.cpu().numpy()
            word_f = word_f.data.cpu().numpy()
            pos_c = pos_c.data.cpu().numpy()
            pos_f = pos_f.data.cpu().numpy()
            ...
            pred_writer.write(sentences, word_c, pos_c, heads_pred, types_pred, lengths, symbolic_root=True)
            gold_writer.write(sentences, word_c, pos_c, heads, types, lengths, symbolic_root=True)

            stats, stats_nopunc, stats_root, num_inst = parser.eval(word_c, pos_c, heads_pred, types_pred, heads, types, word_alphabet, pos_alphabet, lengths, punct_set=punct_set, symbolic_root=True)
