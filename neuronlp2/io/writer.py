...
class CoNLLXWriter(object):
    ...
    def write(self, sentences, word, pos, head, type, lengths, symbolic_root=False, symbolic_end=False):  # not use pos
        ...
        for i in range(batch_size):
            for j in range(start, lengths[i] - end):
                ...
                #
                print(j, sentences[i][j-start][1], sentences[i][j-start][2], sentences[i][j-start][3],\
                      sentences[i][j-start][4], sentences[i][j-start][5], sentences[i][j-start][6], h, t, sentences[i][j-start][9], file=self.__source_file)
                # self.__source_file.write('%d\t%s\t%s\t%s\t%s\t%s\t%d\t%s\t%s\t%s\n' % (j, sentences[i][j-start][1], sentences[i][j-start][2], sentences[i][j-start][3], \
                #                                                                        sentences[i][j-start][4], sentences[i][j-start][5], sentences[i][j-start][6], h, t,  sentences[i][j-start][9]))
            self.__source_file.write('\n')
