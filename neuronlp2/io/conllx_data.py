        ...
        for data_path in data_paths:
            ...
            with open(data_path, 'r') as file:
                for line in file:
                    ...
                    for char in ''.join(tokens[3].split(" ")):  # 191117: 열 ㄴ다 -> 열ㄴ다
                        char_alphabet.add(char)

                    #for pos in tokens[4].split(" "):   #180815 XPOSTAG " " --> "+"
                    for pos in tokens[5].split("+"):
                        pos_alphabet.add(pos)
                    pos_alphabet.add('-')

                    words = utils.DIGIT_RE.sub("0", tokens[3]) if normalize_digits else tokens[3]
                    #pos = tokens[4]
                    type = tokens[8]
    ...
    if not os.path.isdir(alphabet_directory):
        ...
        with open(train_path, 'r') as file:
            for line in file:
                ...
                tokens = line.split('\t')
                # for char in tokens[1]:
                for char in ''.join(tokens[3].split(" ")):  # 191117 tokens[3]: lemma
                    char_alphabet.add(char)

                #for pos in tokens[4].split(" "):   #180815 XPOSTAG " " --> "+"
                # for pos in tokens[4].split("+"):
                for pos in tokens[5].split("+"):  # 191117 tokens[5]: xpostag
                    pos_alphabet.add(pos)
                pos_alphabet.add('-')  # 191117

                # words = utils.DIGIT_RE.sub("0", tokens[2]) if normalize_digits else tokens[2]
                words = utils.DIGIT_RE.sub("0", tokens[3]) if normalize_digits else tokens[3]  # 191117 tokens[3]: lemma
                # pos = tokens[4]
                # type = tokens[7]
