...
class StackPtrNet(nn.Module):
    def __init__(self, word_dim, num_words, char_dim, num_chars, pos_dim, num_pos, char_num_filters, char_kernel_size,
                 ...
                 biaffine=True, pos=True, char=True, eojul=True, prior_order='inside_out', skipConnect=False, grandPar=False, sibling=False):

        super(StackPtrNet, self).__init__()
        self.word_embedd = Embedding(num_words, word_dim, init_embedding=embedd_word)
        self.pos_embedd = Embedding(num_pos, pos_dim, init_embedding=embedd_pos) if pos else None
        self.char_embedd = Embedding(num_chars, char_dim, init_embedding=embedd_char) if char else None
        self.char_conv1d = nn.Conv1d(char_dim, char_num_filters, char_kernel_size, padding=char_kernel_size - 1) if char else None
        eojul_dim = word_dim + char_num_filters + pos_dim   #adding 180809
        # self.eojul_conv1d = nn.Conv1d(eojul_dim, eojul_num_filters, eojul_kernel_size, padding=eojul_kernel_size - 1) if eojul else None  # adding 180809
        # print((int(eojul_num_filters/3)*2), eojul_dim // 1, eojul_kernel_size)
        # print(torch.Tensor(int((eojul_num_filters/3)*2), eojul_dim // 1, eojul_kernel_size))
        self.cont_conv1d = nn.Conv1d(eojul_dim, int((eojul_num_filters/3)*2), eojul_kernel_size, padding=eojul_kernel_size - 1) if eojul else None    #adding 191115
        self.func_conv1d = nn.Conv1d(eojul_dim, int(eojul_num_filters/3), eojul_kernel_size, padding=eojul_kernel_size - 1) if eojul else None   #adding 191111
        self.dropout_in = nn.Dropout2d(p=p_in)
        self.dropout_out = nn.Dropout2d(p=p_out)
        self.num_labels = num_labels
    ...
    def _get_encoder_output(self, input_word_c, input_word_f, input_char_c, input_char_f, input_pos_c, input_pos_f, mask_e=None, length_e=None, hx=None):
        # [2, batch, length, lemma_length, word_dim] == input_word
        # word = self.word_embedd(input_word)
        # apply dropout on input
        # word = self.dropout_in(word)

        # [batch, length, lemma_length, word_dim] == cont
        cont = self.word_embedd(input_word_c)
        cont = self.dropout_in(cont)
        func = self.word_embedd(input_word_f)
        func = self.dropout_in(func)
        ...
        ################################################################################################################
        # content word encoding
        if self.char:
            # [batch, length, char_length, char_dim]
            #print([x for x in input_char.view(-1).numpy()])
            char = self.char_embedd(input_char_c)
            char_size = char.size()
            ...
            char = char.view(char_size[0] * char_size[1] * char_size[2], char_size[3], char_size[4]).transpose(1, 2)
            # put into cnn [batch*length, char_filters, char_length]
            # then put into maxpooling [batch * length, char_filters]
            char = self.char_conv1d(char)
            char = F.elu(char)
            ...
            cont_encoding = torch.cat([cont_encoding, char], dim=3)

        if self.pos:
            # [batch, length, lemma_length, pos_dim]
            pos = self.pos_embedd(input_pos_c)
            # apply dropout on input
            pos = self.dropout_in(pos)
            cont_encoding = torch.cat([cont_encoding, pos], dim=3)
        ...
        if self.eojul:
            ...
            cont_encoding = eojul
        ################################################################################################################
        # functional word encoding
        if self.char:
            # [batch, length, char_length, char_dim]
            #print([x for x in input_char.view(-1).numpy()])
            char = self.char_embedd(input_char_f)
            char_size = char.size()
            # first transform to [batch * length * lemma_length, char_length, char_dim]
            # then transpose to [batch * length * lemma_length, char_dim, char_length]
            char = char.view(char_size[0] * char_size[1] * char_size[2], char_size[3], char_size[4]).transpose(1, 2)
            ...
            char = self.dropout_in(char)
            # concatenate word and char [batch, length, lemma_length, word_dim+char_filter]
            func_encoding = torch.cat([func_encoding, char], dim=3)

        if self.pos:
            # [batch, length, lemma_length, pos_dim]
            pos = self.pos_embedd(input_pos_f)
            # apply dropout on input
            pos = self.dropout_in(pos)
            func_encoding = torch.cat([func_encoding, pos], dim=3)
        ...
        if self.eojul:
            ...
            func_encoding = eojul
        ################################################################################################################
        # print dimension each(cont, func)
        # print("=====invalid argument=====")
        # print(cont_encoding.shape)
        # print(cont_encoding)
        # print(func_encoding.shape)
        # print(func_encoding)
        # src_encoding = torch.stack([cont_encoding, func_encoding])  # cat? stack? will there be any dimension problem?
        src_encoding = torch.cat([cont_encoding, func_encoding], dim=-1)

        # output from rnn [batch, length, hidden_size]
        output, hn = self.encoder(src_encoding, mask_e, hx=hx)
        # print output dimension and compare with original one
    ...
    def loss(self, input_word_c, input_word_f, input_char_c, input_char_f, input_pos_c, input_pos_f, heads, stacked_heads, children, siblings, stacked_types, label_smooth,
        ...
        output_enc, hn, mask_e, _ = self._get_encoder_output(input_word_c, input_word_f, input_char_c, input_char_f, input_pos_c, input_pos_f, mask_e=mask_e, length_e=length_e, hx=hx)
    ...
    def decode(self, input_word_c, input_word_f, input_char_c, input_char_f, input_pos_c, input_pos_f, mask=None, length=None, hx=None, beam=1, leading_symbolic=0, ordered=True):
        ...
        output_enc, hn, mask, length = self._get_encoder_output(input_word_c, input_word_f, input_char_c, input_char_f, input_pos_c, input_pos_f, mask_e=mask, length_e=length, hx=hx)
        # output size [batch, length_encoder, arc_space]
        arc_c = F.elu(self.arc_c(output_enc))
        # output size [batch, length_encoder, type_space]
        type_c = F.elu(self.type_c(output_enc))
